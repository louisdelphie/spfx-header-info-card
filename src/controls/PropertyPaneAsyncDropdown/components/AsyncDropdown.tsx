...
export default class AsyncDropdown extends React.Component<IAsyncDropdownProps, IAsyncDropdownState> {
  ...
  public render(): React.ReactElement {
    const loading: React.ReactElement = this.state.loading ? (
      <div>
    ...
    const error: React.ReactElement =
      this.state.error !== undefined ? <div className={"ms-TextField-errorMessage ms-u-slideDownIn20"}>Error while loading items: {this.state.error}</div> : <div />;
  ...
  private onChanged(event: unknown, option: IDropdownOption, index?: number): void {
